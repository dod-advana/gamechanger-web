...
import { Select, TextField, FormControl, Typography, MenuItem } from '@material-ui/core'
...
export default function UserFeedback (props) {
        ...
        if(screenShot && screenShot !== ''){
            ...
            html2canvas(document.querySelector('#app')).then(async (canvas) => {
                ...
                */
                ...
                try {
                    const imgSrc=' ';
                    await gameChangerAPI.sendFeedbackPOST({feedbackType, feedbackText, screenShot: imgSrc, userEmail: userEmail});
                    setLoading(false);
                    setSuccessFlag(true);
                    setScreenShot('');
                } catch(err) {
    ...
    const renderError = () => {
        return(
            <div style={{display:'flex', justifyContent:'center', flexDirection:'column'}}>
                <ErrorOutlineIcon
                    style={{alignSelf:'center',marginTop:'150px', height:'75px',width:'75px', color: 'red'}}/>

                <h1 style={{marginTop:'50px',alignSelf:'center'}}>Sorry there seems to be an issue on our end. Please try again later</h1>
                <GCButton
                    id={'feedbackClose'}
                    onClick={()=>{
                        closeModal();
    ...
    const renderForm = () => {
        return (
            ...
            <Typography variant="h2" style={{ width: '100%', padding: '20px', paddingLeft: '20px', fontSize:'25px' }}>User Feedback</Typography>
                <div style={{ height: 75 }}>
                    <FormControl variant="outlined" style={{width:'30%', margin: '0px 0px 0px 20px'}}>
                        <Select value={feedbackType} onChange={handleTypeChange} IconComponent={KeyboardArrowDown}>
                            ...
                            <MenuItem value="Other (please provide description below)">Other (please provide description below)</MenuItem>
                        </Select>
                    </FormControl>
                    <TextField
                        variant="outlined"
