...
const StyledTab = styled(Tab)`
	...
	border-radius: ${({first}) => first === 'true' ? PAGE_BORDER_RADIUS : 0}px ${({last}) => last === 'true' ? PAGE_BORDER_RADIUS: 0}px 0px 0px;
	position: relative !important;
	list-style: none !important;
	padding: 2px 12px !important;
	cursor: pointer !important;
	text-align: center;
	background-color: ${({active, primary_color}) => active === 'true' ? primary_color : backgroundWhite} !important;
	margin-right: 2px;
	margin-left: 2px;
...
const TabView = (props) => {
		...
		return (
			<StyledTabsList style={styles.tabsList} primary_color={toolStyle.primaryColor}>
				{tabNames.map((tmpTabName, idx) => {
					return (
						<StyledTab active={tabName === tmpTabName.name ? 'true' : 'false'} title={tmpTabName.name}
							key={tmpTabName.name} first={idx === 0 ? 'true' : 'false'} last={idx === tabNames.length - 1 ? 'true' : 'false'}
							primary_color={toolStyle.primaryColor} className={tmpTabName.className}
						>
							<Typography variant="h6" display="inline" title={tmpTabName.name}>{tmpTabName.title}</Typography>
						</StyledTab>
					)
				})}
			</StyledTabsList>
	...
	return (
		<Tabs selectedIndex={currentTabIndex} onSelect={(tabIndex, lastIndex, event) => handleTabClicked(tabIndex, lastIndex, event.target.title)}>
			...
			{ !state.hideTabs && tabPanels.map(tabPanel => (
				tabPanel
			))}
		</Tabs>
	);
