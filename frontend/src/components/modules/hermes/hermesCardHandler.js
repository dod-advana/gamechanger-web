...
import {
	capitalizeFirst,
	CARD_FONT_SIZE, getSubTypes, getTrackingNameForFactory,
	getTypeDisplay,
...
const renderHighlights = (highlights, capitalizeFirst, hoveredHit, setHoveredHit, setHighlightText) => {
		const fontSize = 12;
		const highlightList = [];
					...
					<>
						<div
							...
							style={{
								...
								...(hoveredHit === highlightLabel && { backgroundColor: '#E9691D', color: 'white' }),
							}}
							onMouseEnter={() => {setHoveredHit(highlightLabel); setHighlightText(text); }}
						>
							<span style={{ fontSize }}>{highlightLabel}</span><i className="fa fa-chevron-right" style={{ marginLeft: 10, fontSize, color: 'white' }} />
						</div>
					</>
				);
			}
				...
				<StyledFrontCardHeader listView={state.listView} docListView={docListView} intelligentSearch={false}>
					...
					{docListView &&
						<div className={'list-view-sub-header'}>
							<p> {getSubTypes(type.toLowerCase())} | {getTypeDisplay(org)} </p>
						</div>
					}
			...
			return (
				<>
					{!state.listView && !toggledMore &&
						<StyledFrontCardSubHeader typeTextColor={'white'} docTypeColor={typeColor} docOrgColor={orgColor}>
				...
				<div style={styles.bodyContainer}>
					...
					{item && item.highlight &&
						<div style={{ display: 'flex', height: '100%', margin: '5px 0 0 0' }}>
							<div style={{  minWidth: 100, height: 190, border: '1px solid rgb(189, 189, 189)', borderTop: 0, overflow: 'scroll' }}>
