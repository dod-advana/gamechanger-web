...
const handlePopPubs = async(pop_pubs, pop_pubs_inactive, state, dispatch) => {
	...
	try {
		...
		for(let i = 0; i < filteredPubs.length; i++){
			gameChangerAPI.thumbnailStorageDownloadPOST([filteredPubs[i]], 'thumbnails', state.cloneData).then((pngs) => {
...
const handleSources = async(state, dispatch) => {
	...
	try {
		...
		for(let i = 0; i < thumbnailList.length; i++){
			gameChangerAPI.thumbnailStorageDownloadPOST([thumbnailList[i]], folder, state.cloneData).then( (pngs) => {
				const buffers = pngs.data;
				buffers.forEach((buf,idx) => {
					if(buf.status === "fulfilled"){
															...
															{activeCategoryTab === 'all' ? <>
																...
																<div className='gcPagination col-xs-12 text-center'>
																	<Pagination
																		...
																		onChange={async page => {
																			trackEvent(getTrackingNameForFactory(cloneData.clone_name), 'PaginationChanged', 'page', page);
