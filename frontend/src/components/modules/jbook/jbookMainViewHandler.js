...
import { getTrackingNameForFactory, scrollToContentTop, getQueryVariable } from '../../../utils/gamechangerUtils';
...
const jbookMainViewHandler = {
	...
	getMainView(props) {
		...
		const { loading, mainPageData, resultsPage, mainTabSelected, exportLoading, feedbackSubmitted, feedbackText } =
			state;
		...
		const getMainPageColumns = () => {
			const mainPageColumns = [
				...
				{
					Header: () => (
						<p style={styles.tableColumn}>
							BUDGET TYPE{' '}
							<span style={styles.orangeText}>
								{state.jbookSearchSettings.budgetType.length === state.defaultOptions.budgetType.length
									? ''
									: `(${state.jbookSearchSettings.budgetType.length})`}
							</span>
						</p>
					),
					Filter: () => (
						<DropdownFilter
							...
							openDropdown={state.budgetTypeDropdown}
							setJBookSetting={setJBookSetting}
							type={'budgetType'}
							options={state.defaultOptions.budgetType}
							clearText={state.jbookSearchSettings.clearText}
						/>
					),
					accessor: 'type',
					width: 130,
				...
				},
				{
					Header: () => (
						<p style={styles.tableColumn}>
							BUDGET YEAR (FY){' '}
							<span style={styles.orangeText}>
								{state.jbookSearchSettings.budgetYear.length === state.defaultOptions.budgetYear.length
									? ''
									: `(${state.jbookSearchSettings.budgetYear.length})`}
							</span>
						</p>
					),
					Filter: () => (
						<DropdownFilter
							setOpenDropdown={setDropdown}
							openDropdown={state.budgetYearDropdown}
							setJBookSetting={setJBookSetting}
							type={'budgetYear'}
							options={state.defaultOptions.budgetYear}
							clearText={state.jbookSearchSettings.clearText}
						/>
					),
					accessor: 'budgetYear',
				...
				},
				{
					...
					accessor: (row) =>
						row.budgetLineItem && row.budgetLineItem !== '-' ? row.budgetLineItem : row.programElement,
					width: 180,
					Cell: (row) => (
						<div style={{ textAlign: 'center' }}>
							<p>{row.value}</p>
				...
				},
				{
					Header: () => (
						<p style={styles.tableColumn}>
							SERVICE / AGENCY{' '}
							<span style={styles.orangeText}>
								{state.jbookSearchSettings.serviceAgency.length ===
								state.defaultOptions.serviceAgency.length
									? ''
									: `(${state.jbookSearchSettings.serviceAgency.length})`}
							</span>
						</p>
					...
					Filter: () => (
						<DropdownFilter
							setOpenDropdown={setDropdown}
							openDropdown={state.serviceAgencyDropdown}
							setJBookSetting={setJBookSetting}
							type={'serviceAgency'}
							options={state.defaultOptions.serviceAgency}
							clearText={state.jbookSearchSettings.clearText}
						/>
					),
				...
				},
				{
					Header: () => (
						<p style={styles.tableColumn}>
							PRIMARY REVIEWER{' '}
							<span style={styles.orangeText}>
								{state.jbookSearchSettings.primaryReviewer.length ===
								state.defaultOptions.primaryReviewer.length
									? ''
									: `(${state.jbookSearchSettings.primaryReviewer.length})`}
							</span>
						</p>
					...
					Filter: () => (
						<DropdownFilter
							setOpenDropdown={setDropdown}
							openDropdown={state.primaryReviewerDropdown}
							setJBookSetting={setJBookSetting}
							type={'primaryReviewer'}
							options={state.defaultOptions.primaryReviewer}
							clearText={state.jbookSearchSettings.clearText}
						/>
					),
				...
				},
				{
					Header: () => (
						<p style={styles.tableColumn}>
							SERVICE REVIEWER{' '}
							<span style={styles.orangeText}>
								{state.jbookSearchSettings.serviceReviewer.length ===
								state.defaultOptions.serviceReviewer.length
									? ''
									: `(${state.jbookSearchSettings.serviceReviewer.length})`}
							</span>
						</p>
					...
					Filter: () => (
						<DropdownFilter
							setOpenDropdown={setDropdown}
							openDropdown={state.serviceReviewerDropdown}
							setJBookSetting={setJBookSetting}
							type={'serviceReviewer'}
							options={
								state.dropdownData && state.dropdownData.serviceReviewers
									? state.dropdownData.serviceReviewers.map(
											(r) => `${r.name}${r.organization ? ` (${r.organization})` : ''}`
									  ) ?? []
									: []
							}
							secondaryOptions={
								state.dropdownData && state.dropdownData.secondaryReviewers
									? state.dropdownData.secondaryReviewers.map(
											(r) => `${r.name}${r.organization ? ` (${r.organization})` : ''}`
									  )
									: []
							}
				...
				},
				{
					Header: () => (
						<p style={styles.tableColumn}>
							REVIEW STATUS{' '}
							<span style={styles.orangeText}>
								{state.jbookSearchSettings.reviewStatus.length ===
								state.defaultOptions.reviewStatus.length
									? ''
									: `(${state.jbookSearchSettings.reviewStatus.length})`}
							</span>
						</p>
					...
					Filter: () => (
						<DropdownFilter
							right={'5px'}
							setOpenDropdown={setDropdown}
							openDropdown={state.reviewStatusDropdown}
							setJBookSetting={setJBookSetting}
							type={'reviewStatus'}
							options={state.defaultOptions.reviewStatus}
							clearText={state.jbookSearchSettings.clearText}
						/>
					),
					Cell: (row) => {
				...
				},
				{
					Header: () => (
						<p style={styles.tableColumn}>
							HAS KEYWORDS{' '}
							<span style={styles.orangeText}>
								{state.jbookSearchSettings.hasKeywords.length === 2
									? ''
									: `(${state.jbookSearchSettings.hasKeywords.length})`}
							</span>
						</p>
					),
					Filter: () => (
						<DropdownFilter
							right={'5px'}
							setOpenDropdown={setDropdown}
							openDropdown={state.hasKeywordsDropdown}
							setJBookSetting={setJBookSetting}
							type={'hasKeywords'}
							options={['Yes', 'No']}
							clearText={state.jbookSearchSettings.clearText}
				...
				},
				{
					Header: () => (
						<p style={styles.tableColumn}>
							LABEL(S){' '}
							<span style={styles.orangeText}>
								{state.jbookSearchSettings.primaryClassLabel.length ===
								state.defaultOptions.primaryClassLabel.length
									? ''
									: `(${state.jbookSearchSettings.primaryClassLabel.length})`}
							</span>
						</p>
					...
					Filter: () => (
						<DropdownFilter
							right={'5px'}
							setOpenDropdown={setDropdown}
							openDropdown={state.primaryClassLabelDropdown}
							setJBookSetting={setJBookSetting}
							type={'primaryClassLabel'}
							options={state.defaultOptions.primaryClassLabel}
							clearText={state.jbookSearchSettings.clearText}
						/>
					),
					width: 150,
						...
						<div style={{ textAlign: 'center' }}>
							<p>
								{row.value === 'Finished Review'
									? row.original.pocClassLabel ??
									  row.original.serviceClassLabel ??
				...
				},
				{
					Header: () => (
						<p style={styles.tableColumn}>
							SOURCE{' '}
							<span style={styles.orangeText}>
								{state.jbookSearchSettings.sourceTag.length === state.defaultOptions.sourceTag.length
									? ''
									: `(${state.jbookSearchSettings.sourceTag.length})`}
							</span>
						</p>
					),
					Filter: () => (
						<DropdownFilter
							right={'5px'}
							setOpenDropdown={setDropdown}
							openDropdown={state.sourceTagDropdown}
							setJBookSetting={setJBookSetting}
							type={'sourceTag'}
							options={state.defaultOptions.sourceTag}
							clearText={state.jbookSearchSettings.clearText}
						/>
					),
					accessor: 'sourceTag',
				...
				<>
					...
					<Grid container style={{ background: '#f2f2f2', borderRadius: 6, marginTop: 10, padding: 10 }}>
						<Grid item xs={11}>
							...
							>
								Service Reviewer O&M Form
							</Typography>
							<Typography style={{ fontFamily: 'Montserrat', fontSize: 16 }}>
								Please use the link provided below to access the Service Reviewer O&M form.
							</Typography>
							<Typography style={{ fontFamily: 'Montserrat', fontSize: 16 }}>
								*Users required to submit an O&M form have been notified of the requirement. Please do
								not complete an O&M form unless requested to do so.
							</Typography>
						</Grid>
						<Grid item xs={1} style={{ alignSelf: 'center' }}>
							<Link
								underline={'none'}
								href={
									'https://forms.osi.apps.mil/Pages/ResponsePage.aspx?id=kQEtEK7uYUexyxqD6G70ReEn65mxjY9GoAbY9R1q21tURUw2OERBOU41R1RROTJVMkZWNUNIM01ZSi4u'
								}
								target="_blank"
							>
								<Typography style={{ fontSize: 20, fontWeight: 500 }} color={'inherit'}>
									O&M Form >
								</Typography>
							</Link>
						</Grid>
					...
					<StyledMainTopBar id="game-changer-content-top">
						<div style={styles.titleText}>
							...
							{mainPageData && mainPageData.totalCount
								? `(${mainPageData.totalCount})`.replace(/\B(?=(\d{3})+(?!\d))/g, ',')
								: ''}
						</div>
						<div className="gcPagination" style={{ display: 'flex', alignItems: 'center' }}>
								...
								onClick={async () => {
									try {
										...
										await autoDownloadFile({
											data: blob,
											extension: 'csv',
											filename: 'checklist-data-' + d.toISOString(),
										});
							...
							<GCPrimaryButton
								...
								onClick={() => {
									setJBookSetting('clearDataSources', '', state, dispatch);
								}}
							>
								Clear Filters
							</GCPrimaryButton>
								...
								return {
									...
									onClick: () => {
										...
										let url = `#/jbook/profile?title=${projectTitle}&programElement=${programElement}&projectNum=${projectNum}&type=${encodeURIComponent(
											budgetType
										)}&budgetLineItem=${budgetLineItem}&budgetYear=${budgetYear}&searchText=${searchText}&id=${id}&appropriationNumber=${appropriationNumber}`;
						...
						<Grid item xs={12}>
							<Grid container className={'summaryTextSection'}>
								<Grid item xs={8}>
									...
									<Typography className={'summarySectionText'}>
										As part of the FY21 Defense Appropriations Bill, Congress tasked the Director of
										the DoD Joint AI Center to provide the congressional defense committees an
										inventory of all DoD artificial intelligence activities, to include each:
									</Typography>
									<ul className={'summarySectionList'}>
										...
										<li>
											the identification of academic or industry mission partners, if applicable
										</li>
										<li>any planned transition partners, if applicable</li>
									</ul>
									<Typography className={'summarySectionText'}>
										Over the past year, JAIC has worked closely with OUSD(R&E), OUSD(C), the
										Services, and other DoD Component representatives of the DoD AI Working Group in
										scoping and executing this task. We delivered the Phase I Inventory of DoD AI
										Programs in April 2021.
									</Typography>
									<Typography className={'summarySectionText'}>
										In partnership with OUSD(C), the JAIC is executing the Phase II AI Inventory via
										the Advana Platform, which the Deputy Secretary of Defense has officially
										designated “as the single enterprise authoritative data management and analytics
										platform.”
									</Typography>
								</Grid>
							...
							<div className={'summaryTextSection'}>
								<Typography className={'summarySectionText'}>
									Using the Advana Gamechanger platform, you will be able to easily review key budget
									and contracting data about AI-related programs and projects and input additional
									information needed to complete this congressionally mandated tasking.
								</Typography>
								<Typography className={'summarySectionText'}>
									Phase II of the AI Inventory (to be submitted to Congress in early 2022) will result
									in a tool that addresses limitations of Phase I deliverable and includes an
									inventory of classified programs. The Phase II inventory will be conducted via a
									three-part data coordination and review process: at the JAIC-level, Service-level,
									and program POC-level.
								</Typography>
								<Typography className={'summarySectionText'}>
									JAIC-Level Review: The JAIC will publish and refine the SIPR and NIPR tools, and
									review the inventory data to initially classify projects and programs as either
									AI-core, AI-enabled, or AI-enabling.
								</Typography>
								<Typography className={'summarySectionText'}>
								...
								<Typography className={'summarySectionText'}>
									...
									program and project, to include: Joint Capability Area, Type of AI, AI domain &
									task.
								</Typography>
								<Typography className={'summarySectionText'}>
									After all phases of coordination and review, the final PAT and Phase II of the
									inventory will be submitted to Congress in early 2022. After that time, this dataset
									will be made available to the entire DoD AI community for use in their analysis and
									planning.
								</Typography>
							</div>
						</Grid>
							...
							<div style={styles.tabButtonContainer}>
								<TabList style={styles.tabsList}>
									<div style={{ flex: 1, display: 'flex' }}>
										...
										>
											<Typography variant="h6" display="inline" title="summaryFAQ">
												SUMMARY AND FAQ
											</Typography>
										</Tab>
										<Tab
										...
										>
											<Typography variant="h6" display="inline" title="reviewerChecklist">
												REVIEWER CHECKLIST
											</Typography>
										</Tab>
									</div>
								...
								<div style={styles.panelContainer}>
									<TabPanel>{renderSummaryAndFAQ()}</TabPanel>
									<TabPanel>{renderMainContainer({ state })}</TabPanel>
								</div>
							</div>
						</Tabs>
					</StyledMainContainer>
