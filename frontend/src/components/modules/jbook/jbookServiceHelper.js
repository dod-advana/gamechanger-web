...
const ReviewStatus = React.memo((props) => {
	...
	return (
		<div style={{ margin: '0 0 15px 0' }}>
			<Typography variant="subtitle1" style={{ color: finished ? 'green' : '#F9B32D', fontSize: '18px', textAlign: 'right' }}>{reviewStatus}</Typography>
		</div>
	);
});
...
const LabelingValidationKey = React.memo(() => {
	...
	return (
		<StyledTableKeyContainer>
			<strong>Labeling Validation<Tooltip
			...
			><InfoOutlinedIcon style={{ margin: '-2px 5px' }} /></Tooltip>
			</strong>
		...
		<StyledTableValueContainer>
			...
			<StyledInlineContainer>
				...
				<Autocomplete
					...
					value={(serviceClassLabel || primaryClassLabel) ? { primary_class_label: serviceClassLabel ?? primaryClassLabel } : null}
					disabled={finished || (!finished && serviceAgreeLabel === 'Yes')} //|| roleDisabled}
					disableClearable
					classes={{ inputRoot: !serviceValidated && !serviceValidation.serviceClassLabel && serviceAgreeLabel === 'No' ? classes.autocompleteError : '' }}
				/>
			</StyledInlineContainer>
		</StyledTableValueContainer>
	);
		...
		<StyledTableValueContainer>
			...
			<StyledInlineContainer>
				...
				<Autocomplete
					...
					value={(servicePlannedTransitionPartner || primaryPlannedTransitionPartner) ? servicePlannedTransitionPartner ?? primaryPlannedTransitionPartner : null}
					disabled={finished || (!finished && servicePTPAgreeLabel === 'Yes')} //|| roleDisabled}
					disableClearable
					classes={{ inputRoot: !serviceValidated && !serviceValidation.servicePlannedTransitionPartner && servicePTPAgreeLabel === 'No' ? classes.autocompleteError : '' }}
				/>
			</StyledInlineContainer>
		</StyledTableValueContainer>
	);
...
const MissionPartnersValue = React.memo((props) => {
	...
	useEffect(() => {
		if (Object.prototype.toString.call(serviceMissionPartnersChecklist) === '[object Object]') {
			...
			setMissionPartnersChecklist(serviceMissionPartnersChecklist ? JSON.parse(serviceMissionPartnersChecklist) : {});
		}
	}, [serviceMissionPartnersChecklist]);
		...
		<StyledTableValueContainer>
			...
			</StyledInlineContainer>
			{
				renderMissionPartnersCheckboxes(
					(value) => { setReviewData('setMissionPartnersChecklist', value) },
...
const AIPOCKey = React.memo(() => {
	...
	return (
		<StyledTableKeyContainer>
			...
			<Typography variant="subtitle1" style={{ fontSize: 12 }}>Enter the AI Point of Contact for this Program/Project in the POC section of the Service Reviewer Section. A suitable type of POC would be the Program Element Monitor. We ask that you enter the POC Title, Name, Email address, Organization and Phone number in this section.</Typography>
		</StyledTableKeyContainer>
	);
		...
		<StyledTableValueContainer>
			<StyledInlineContainer justifyContent={'left'}>
				...
				<TextField
					...
					size="small"
					disabled={finished} //|| roleDisabled}
					InputProps={!serviceValidated && !serviceValidation.pocTitle ? {
						classes: {
			...
			<StyledInlineContainer justifyContent={'left'}>
				...
				<TextField
					...
					size="small"
					disabled={finished} //|| roleDisabled}
					InputProps={!serviceValidated && !serviceValidation.pocName ? {
						classes: {
							root: classes.cssOutlinedInput,
							focused: classes.cssFocused,
							notchedOutline: classes.notchedOutline
			...
			<StyledInlineContainer justifyContent={'left'}>
				...
				<TextField
					...
					size="small"
					disabled={finished} //|| roleDisabled}
					InputProps={!serviceValidated && !serviceValidation.pocEmail ? {
						classes: {
							root: classes.cssOutlinedInput,
							focused: classes.cssFocused,
							notchedOutline: classes.notchedOutline
			...
			<StyledInlineContainer justifyContent={'left'}>
				...
				<TextField
					...
					size="small"
					disabled={finished} //|| roleDisabled}
					InputProps={!serviceValidated && !serviceValidation.pocOrg ? {
						classes: {
							root: classes.cssOutlinedInput,
							focused: classes.cssFocused,
							notchedOutline: classes.notchedOutline
			...
			<StyledInlineContainer justifyContent={'left'}>
				...
				<TextField
					...
					size="small"
					disabled={finished} //|| roleDisabled}
					InputProps={!serviceValidated && !serviceValidation.pocPhoneNumber ? {
						classes: {
							root: classes.cssOutlinedInput,
							focused: classes.cssFocused,
							notchedOutline: classes.notchedOutline
...
const ReviewerNotesKey = React.memo(() => {
	...
	return (
		<StyledTableKeyContainer>
			...
			<Typography variant="subtitle1" style={{ fontSize: 12 }}>Please add any other relevant information to the Reviewer Notes Text Box.</Typography>
		</StyledTableKeyContainer>
	);
...
const ServiceDescriptionText = React.memo(() => {
	return (
		<StyledTableValueContainer>
			<Typography variant="subtitle1" style={{ fontSize: 12 }}>Once your review is complete, click the submit finished review button to save your entries/information. You can also Save a partial review to finish later by clicking the Save Partial Review button or you can reset the Service Reviewer Section to blank values by clicking the reset Form Buttons.</Typography>
			<hr />
	...
	return (
		<StyledFooterDiv>
			...
			<Tooltip
				placement="top"
				arrow
				title={'This button will reset the service level review with blank values'}
			>
				<GCPrimaryButton
					style={{ color: '#515151', backgroundColor: '#E0E0E0', borderColor: '#E0E0E0', height: '35px' }}
					onClick={() => {
			...
			<Tooltip
				placement="top"
				arrow
				title={'This will save a partial review to finish later'}
			>
				<GCPrimaryButton
					style={{ color: '#515151', backgroundColor: '#E0E0E0', borderColor: '#E0E0E0', height: '35px' }}
