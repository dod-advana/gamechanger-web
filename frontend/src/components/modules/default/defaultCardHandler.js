...
import {
	...
	orgAlias, getMetadataForPropertyTable, encode,
...
const clickFn = (filename, cloneName, searchText, pageNumber = 0) => {
	...
	window.open(`/#/pdfviewer/gamechanger?filename=${encode(filename)}&prevSearchText=${searchText.replace(/"/gi, '')}&pageNumber=${pageNumber}&cloneIndex=${cloneName}`);
};
...
const DefaultCardHandler = {
	...
	entity: {
			...
			return (
				<StyledFrontCardHeader listView={state.listView} docListView={state.listView} intelligentSearch={false}>
					<div className={'title-text-selected-favorite-div'}>
						<GCTooltip title={displayTitle} placement='top' arrow>
							<div className={'title-text'}
								 onClick={(state.listView) ? () => window.open(`#/gamechanger-details?type=entity&entityName=${item.name}&cloneName=${state.cloneData.clone_name}`) : () => {}}
							>
								<div className={'text'}>
									{displayTitle}
								</div>
		...
		getCardBack: (props) => {
			...
			metadata.push({
				Key: 'Website',
				Value: <a href={item.resource_url} target="_blank" rel="noopener noreferrer">{item.resource_url}</a>
			});
			...
			return (
				<>
					<>
						<CardButton target={'_blank'} style={{...styles.footerButtonBack, CARD_FONT_SIZE}} href={'#'}
							onClick={(e) => {
								trackEvent(getTrackingNameForFactory(cloneName), 'GraphCardInteraction', 'Open', `${name}DetailsPage`);
									e.preventDefault();
									window.open(`#/gamechanger-details?type=entity&entityName=${name}&cloneName=${cloneName}`);
							}}
						>
							Open
						</CardButton>
