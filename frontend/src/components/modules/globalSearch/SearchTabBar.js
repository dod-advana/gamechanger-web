...
const SearchTabBar = (props) => {
	...
	return <div style={{...styles.container, ...containerStyles}}>
		...
		<div style={styles.tabsContainer}>
			{_.map(searchTypesWithAll, (enabled, searchType) => {
				...
				return <Typography key={searchType} style={style} variant="body1" onClick={() => setActiveTab(searchType)}>
					{formatLabelFromKey(searchType)}{formatMetaData(resultMetaData, searchType)}
				</Typography>
			})}
		</div>
	</div>
