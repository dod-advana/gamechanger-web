...
import { SearchBarForm, SearchBarInput, SearchButton, AdvancedSearchButton } from '../../searchBar/SearchBarStyledComponents';
...
const EDASearchBarHandler = {
	async debouncedFetchSearchSuggestions(value, state, setAutocorrect, setPresearchTitle, setPresearchTopic, setPresearchOrg, setPredictions) {
		try {
		...
		return (
			<div style={{ display: 'flex', justifyContent: 'center', width: '100%', position: "relative" }} ref={ref}>
				...
				>
					...
					{ hideSearchResults && <AdvancedDropdown
                        context={context}
					...
					{ hideSearchResults && <AdvancedSearchButton type='button' id='advancedSearchButton' onClick={() => {setAdvancedSearchOpen(!advancedSearchOpen)}}>
                        Advanced
