	...
	async getPresearchData(state, dispatch) {
		...
		if (!state.filterDataFetched) {
			...
			const newFilterData = {
				...
				fundingOfficeDoDAAC: resp.data.fpds_funding_office_code.map((e) => e.toUpperCase()).sort(),
				fundingAgencyName: resp.data.fpds_funding_agency_name.map((e) => e.toUpperCase()).sort(),
				psc: resp.data.fpds_psc.map((e) => e.toUpperCase()).sort(),
				naics: resp.data.fpds_naics_code.map((e) => e.toUpperCase()).sort(),
				duns: resp.data.fpds_duns.map((e) => e.toUpperCase()).sort(),
				modNumber: resp.data.fpds_modification_number.map((e) => e.toUpperCase()).sort(),
			};
