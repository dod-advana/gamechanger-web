...
const GroupCard = (props) => {
	const { group, idx, state, dispatch, favorites } = props;
		...
		<StyledFavoriteGroupCard key={idx}>
			<div style={styles.exportButton}>
				<SelectedDocsDrawer
					...
					openExport={() => {
						setState(dispatch, {
							selectedDocuments: favFilenames,
							exportDialogVisible: true,
							prevSearchText: combinedSearchText,
						});
					}}
			...
			<div style={styles.titleSection}>
				...
				<div style={styles.groupDescription}>{group.group_description}</div>
			</div>
			{favorites.length > 0 ? (
				...
				<div style={styles.placeHolder}>
					Go to the favorites tab to add up to five {group.group_type}s to the group.
				</div>
			)}
			<div style={styles.details}>
				<div style={styles.groupDetails}>{`${favorites.length} items`}</div>
			</div>
