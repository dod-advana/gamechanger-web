...
const SourceDetailsPage = (props) => {
	...
	const upperSource = source.split(' ')
			...
			<MainContainer>
								...
								{source ?
									<>
										<SimpleTable tableClass={'magellan-table'}
											 ...
											 rows={[{key:'Total Documents', value:initialSourceData.totalCount}]|| []}
											 height={'auto'}
											 dontScroll={true}
											 colWidth={colWidth}
											 disableWrap={true}
				...
				<div className={'graph-top-docs'}>
					<div className={'section'}>
						<GCAccordion expanded={true} header={'DOCUMENTS FROM SOURCE'}
									 backgroundColor={'rgb(238,241,242)'}>
							<div style={{width:'100%'}}>
								<div style={{display: 'flex', justifyContent: 'space-between'}}>
									<div
									style={styles.resultsCount}>{initialSourceData.totalCount > 0 ?
									`${numberWithCommas(initialSourceData.totalCount)} results found in ${initialSourceData.timeFound} seconds` : ''}</div>
