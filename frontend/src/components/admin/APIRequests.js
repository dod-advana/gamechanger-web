...
const EditableCell = (props) => {
	...
	return <input onChange={onChange} onBlur={onBlur} value={value} />;
};
...
export default () => {
			...
			Cell: (row) => (
				<TableRow>
					{EditableCell({
						...row,
						handleChange,
						value: keyDescriptions ? keyDescriptions[row.value] : '',
						key: row.value,
					})}
				</TableRow>
			),
	...
	const pendingColumns = [
		...
		},
		{
			...
			id: 'Approve',
			accessor: (request) => {
				return { id: request.id, username: request.username };
			},
			width: 230,
			Cell: (row) => (
				<TableRow>
				...
				<div style={{ padding: '0px 15px 10px' }}>
					...
					<div style={{ width: 350, margin: 5 }}>
						<div style={{ margin: '65px 15px 0' }}>
							User already has an API key. Either reject the request or revoke the user's current API key.
						</div>
						<div style={{ display: 'flex', justifyContent: 'flex-end' }}>
							<GCButton
			...
			<div style={{ margin: '10px 80px' }}>
				...
				<GCAccordion expanded={true} header={'PENDING API KEYS'}>
					<ReactTable
						...
						getTheadThProps={() => {
							return {
								style: { fontSize: 15, fontWeight: 'bold', borderBottom: 'rgba(0,0,0,0.05) 1px solid' },
							};
						}}
					/>
				</GCAccordion>
