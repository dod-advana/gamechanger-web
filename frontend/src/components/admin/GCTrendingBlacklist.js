...
export default () => {
	...
	return (
		<PageWrapper>
			<Typography variant="h2" style={{ width: '100%', marginLeft: '20px', fontSize:'25px' }}>{'Edit Trending Blacklist'}</Typography>
			<div style={{display: 'flex', flexDirection: 'row', marginBottom: '10px'}}>
				<TextField
					...
					margin="dense"
				/>
				<button onClick={handleCreate} style={{ backgroundColor: 'green', color: 'white', border: 'none', borderRadius: '6px', marginTop:'20px', marginLeft: '20px' }}>CREATE</button>
			</div>
				...
				{blacklist.map(({ search_text, added_by }) => {
					return (
						<ListItem key={search_text}>
							...
							<DeleteButton style={{width: '10%'}} onClick={() => handleDelete(search_text)}>Delete</DeleteButton>
						</ListItem>
					)
				})}
			</ListWrapper>
