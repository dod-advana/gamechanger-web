...
class ResponsibilityAssist extends Component {
	...
	componentDidUpdate(prevProps, prevState, snapshot) {
		if (
			this.props.context.state.showResponsibilityAssistModal !==
			prevProps.context.state.showResponsibilityAssistModal
		) {
			if (this.props.context.state.showResponsibilityAssistModal) {
				this.getAnnotationData();
			}
			this.setState({
				isModalOpen: this.props.context.state.showResponsibilityAssistModal,
	...
	handlePreviousNext = (change) => {
		...
		if (change > 0) {
			...
			//previous, viewing 1 more before
			if (startPar !== 0) {
...
*/
	...
	render() {
		const { loading, componentStepNumbers } = this.state;
