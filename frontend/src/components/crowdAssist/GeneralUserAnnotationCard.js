...
export const GeneralUserAnnotationCard = ({ paragraph, tags, tag, entities, componentStepNumbers, entityAnswer, colorMap, tagDescriptions, handleAnswer }) => {
	...
	useEffect(() => {
		...
		tags.forEach(tagText => {
			if (tagText === tag) {
				options.push(<CustomFormControlLabel
					control={<StyledRadio/>}
					key={tagText}
					value={tagText}
					label={
						...
						color: 'white',
						display: 'flex'
					}}>{tagText} <div style={{fontSize: 10, marginLeft: 5}}>{tagDescriptions[tagText]}</div></span>}
	...
	return (
		<div className={`tutorial-step-${componentStepNumbers["Crowd Assist Panel"]}`}>
			...
			<div style={{ backgroundColor: '#DFE6EE', padding: '20px 15px', marginBottom: '15px'}}>
				<p style={{ margin: 0 }}>
				...
				<ol style={{ margin: 0, padding: '0 18px', fontWeight: 600}}>
					<li>
						Click "Yes" if the word(s) is/are tagged correctly.
					</li>
					<li>
						Click "No" if the word(s) is/are not tagged correctly. Then select why using the drop down menu that appears.
					</li>
					<li>
						Click "Not Sure" if unsure.
					</li>
					<li>
						Submit Assist to save the assist and show another.
					</li>
					<li>
			...
			<div style={{ margin: '10px 0 '}}>
				<Typography variant="h5" display="inline">Is the highlighted text in the paragraph below an entity of the type:</Typography>
				<div style={{display: 'flex', marginTop: '5px' }}>
					<FormControl className={`tutorial-step-${componentStepNumbers["Tag Selection"]}`} component="fieldset" style={{width: '100%', paddingTop: '2px', maxWidth: 600 }}>
						<RadioGroup
							row
				...
				<div style={{display: 'flex', marginTop: '10px', marginLeft: '-10px'}}>
						<GCButton
							...
							style={{ height: 40, minWidth: 70, boxShadow: answered ? !entityIncorrect && !entityUnknown ? `0 0 14px 3px ${tertiaryGreen}` : null : null }}
						>
							Yes
						</GCButton>
						<GCButton
							...
							style={{ height: 40, minWidth: 70, boxShadow: answered ? entityIncorrect && !entityUnknown ? `0 0 14px 3px ${tertiaryRed}` : null : null }}
						>
							No
						</GCButton>
						<GCButton
							...
							style={{ height: 40, minWidth: 150, boxShadow: answered ? entityUnknown ? `0 0 14px 3px ${secondaryAlt}` : null : null }}
						>
							Not Sure
						</GCButton>
					</div>
				...
				<div style={{ marginTop: '10px' }}>
					<Typography variant="h5" display="outline">Please choose why the highlighted text is incorrect:</Typography>
					<FormControl variant="outlined" className={classes.formControl}> 
						...
						>
							...
							<MenuItem value={1} classes={{root: {fontSize: '16px'}}}>Text is an entity but classified wrong</MenuItem>
							<MenuItem value={2} classes={{root: {fontSize: '16px'}}}>Text is an entity but parts are missing</MenuItem>
							<MenuItem value={3} classes={{root: {fontSize: '16px'}}}>This is not an entity</MenuItem>
						</Select>
					</FormControl>
				</div>
				}
			...
			<div style={{ border: '2px solid #B0BAC5', boxShadow: '1px 1px gray', borderRadius: '5px', height: 300 }}>
				<TokenAnnotator
					style={{
						lineHeight: 1.5,
						height: '100%',
