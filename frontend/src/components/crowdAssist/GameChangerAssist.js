...
class GameChangerAssist extends Component {
	...
	componentDidUpdate(prevProps, prevState, snapshot) {
		if (this.props.context.state.showAssistModal !== prevProps.context.state.showAssistModal) {
			if ( this.props.context.state.showAssistModal) { this.getAnnotationData(); }
			this.setState({
				isModalOpen: this.props.context.state.showAssistModal,
				voluntary: this.props.context.state.assistVoluntary,
				componentStepNumbers: this.props.context.state.componentStepNumbers,
	...
	getAnnotationData = async () => {
		...
		while(tryCount < maxTries && !goodResponse) {
			let resp = await gameChangerAPI.getDocumentsToAnnotate({ cloneData: this.props.context.state.cloneData});
	...
	setupGeneralUserData = (data) => {
		...
		this.setState({
			...
			progressText: `${(this.state.currentEntityIndex + 1)} / ${paragraphEntities[randomParagraphIndex].length} entities left in this paragraph`,
	...
	handleChangeEntity = (change) => {
		const { currentEntityIndex, paragraphEntities, currentParagraphIndex, paragraphEntityAnswers, canMoveForward } = this.state;
		...
		this.setState({
			...
			progressText: `${(newEntityIndex + 1)} / ${paragraphEntities[currentParagraphIndex].length} entities left in this paragraph`,
	...
	handleGeneralAnswers = (correct, incorrectReason, unknown= false) => {
		...
		paragraphEntityAnswers[currentParagraphIndex][currentEntityIndex] = { correct, incorrectReason, unknown, answered: true };
