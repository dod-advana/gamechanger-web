...
const Alerts = (props) => {
	...
	return (
		<>
			{
				alerts.noResultsMessage && <UoTAlert
					title={'No results'}
					top={140}
					type={'warning'}
			...
			{
				alerts.unauthorizedError && <UoTAlert
					title={'Error communicating with the API'}
					type={'error'}
					top={140}
					message={alerts.unauthorizedError}
					onHide={() => dispatch({type: 'SET_ALERT', payload: { unauthorizedError: false }})}
					containerStyles={styles.alertStyle}
				/>
			}
			{
				alerts.transformFailed && <UoTAlert
