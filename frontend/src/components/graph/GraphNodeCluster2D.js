...
export default function GraphNodeCluster2D(props) {
	...
	const {
		...
		graphHeight = window.innerHeight * .75,
	...
	useEffect(() => {
		...
		if (graph.edges) {
			...
			Object.keys(edgeLabels).forEach((label, idx) => {
				tmpEdgeLabelPatterns[label] = { label: label, pattern: edgePatterns[idx] || []};
			});
	...
	const handleNodeClick = onNodeClick ? onNodeClick : async (node, event) => {
	...
	const handleNodeDrag = onNodeDrag ? onNodeDrag : (node, translate) => {
		setShouldRunSimulation(true);
	}
	
	const handleNodeDragEnd = onNodeDragEnd ? onNodeDragEnd : (node, translate) => {
		setShouldRunSimulation(false);
	}
	
	const handleBackgroundClick = onBackgroundClick ? onBackgroundClick : (event) => {
		setSelectedNodeID(-1);
	}
	...
	const handleRenderEdgeLegendItems = renderEdgeLegendItems ? renderEdgeLegendItems : () => {
		return (
			<>
				{!runningQuery &&
	...
	const handleRenderLegend = () => {
		return (
			<div style={styles.legendKey}>
				...
				<div style={{borderBottom: '1px solid black', width: '80%', marginLeft: '10%', marginTop: '1em'}}/>
	...
	const settingsBurger = () => {
		return (
			<StyledBurger open={settingsOpen} onClick={() => {
				trackEvent(getTrackingNameForFactory(cloneData.clone_name), 'GraphSettingsMenuOnClick', !settingsOpen ? 'open' : 'close');
