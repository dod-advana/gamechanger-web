...
export default class SimpleTable extends React.Component {
	...
	getBody = (rows, cols, colMap, onRowClick) => {
		...
		const { highlightSelectedRow, colWidth, firstColWidth, returnRowOnClick, useParser } = this.props;
		const tbodyStyle = {
		...
		for (let rIdx = 0; rIdx < rows.length; rIdx++) {
			...
			if (colMap) {
				_.each(colMap, (c, cIdx) => {
					...
					rowCells.push(<td title={c.noTooltip ? '' : innerVal} style={{ ...colWidth, ...extraWrapStyle }} key={`${rIdx}_${cIdx}`}>{innerVal}</td>)
				})
			}
			else {
				...
				_.each(cols, (c, cIdx) => {
					let value = r[c];
					const editIcon = this.props.showEditIcon && cIdx === 0 && <i style={{ marginRight: 10, color: 'blue' }} className="fa fa-pencil" />
					if (!value || _.isBoolean(value)) value = '';
					rowCells.push(<td title={value} style={{ ...(cIdx === 0 ? firstColWidth : colWidth), ...extraWrapStyle }} key={`${rIdx}_${cIdx}`}>{editIcon}{(useParser) ? parse(r[c]) : r[c]}</td>)
				})
			}
			...
			rowItems.push(<tr style={isSelectedRow ? activeRow : {}} onClick={() => {
			...
			}} key={rIdx}>{rowCells}</tr>)

			if (onGoingCellCount > CONFIG.MAX_SIMPLE_TABLE_CELLS)
				break;
		};
	...
	render() {
		const { rows, colKeys, columnMap, onRowClick, height, zoom, tableClass, dontScroll, inheritOverflow, margin } = this.props;
