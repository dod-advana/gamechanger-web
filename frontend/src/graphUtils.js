...
export const draw2DArrows = (link, ctx, globalScale, arrowLength, arrowRelativePos, color, nodeRelSize, test = false) => {
	...
	if (!start || !end || !start.hasOwnProperty('x') || !end.hasOwnProperty('x') || !start.x || !start.y || !end.x || ! end.y) return; // skip invalid link
	...
	if (test) {
		return {posAlongLine, arrowHead, arrowTail, arrowTailVertex, arrowTailAngle};
	}
...
export const  getNodeColors = (node, alpha, nodeLabelColors = {}) => {
	...
	if (nodeLabelColors[node?.label] && nodeLabelColors[node?.label].color === '') {
		...
		Object.keys(nodeLabelColors).forEach((label, idx) => {
			nodeLabelColors[label] = {color: NODE_COLORS[idx].color, textColor: NODE_COLORS[idx].textColor};
		});
	} else {
		if (!nodeLabelColors[node?.label] || nodeLabelColors[node?.label].color === '') {
			return {
				nodeColor: convertHexToRgbA('#000000', alpha),
				nodeTextColor: convertHexToRgbA('#ffffff', alpha), nodeHexColor: '#000000'
			};
		}
	}
	...
	return { nodeColor: convertHexToRgbA(nodeColor, alpha),
			nodeTextColor: convertHexToRgbA(getTextColorBasedOnBackground(nodeColor), alpha), nodeHexColor: nodeColor };
}
...
export function generateRandomColors (number) {
    let i;
    if (typeof(arguments[1]) != 'undefined' && arguments[1].constructor === Array &&arguments[1][0] &&
		arguments[1][0].constructor !== Array) {
        for(i = 0; i<arguments[1].length; i++){ //for all the passed colors
	...
	const shiftHue = (rgb,degree) => {//shifts [r,g,b] by a number of degrees
	...
	let differenceRecursions = {//stores recursion data, so if all else fails we can use one of the hues already generated
		differences:[],//used to calculate the most distant hue
		values:[]//used to store the actual colors
	};
	
	const fixDifference = (colorMap = []) => {//recursively asserts that the current color is distinctive
		if (differenceRecursions.values.length > 23) {//first, check if this is the 25th recursion or higher. (can we try any more unique hues?)
			...
			const ret = differenceRecursions.values[differenceRecursions.differences.indexOf(Math.max.apply(null,differenceRecursions.differences))];
		...
		for (let i=0; i<loadedColors.length; i++) { //for all the colors we've generated so far
			...
			const sumFunction = (sum,value) => { //function for adding up arrays
				return sum+value
			}
	...
	const color = () => { //generate a random color
		const scale = (x) => { //maps [0,1] to [300,510]
			return x*210+300 //(no brighter than #ff0 or #0ff or #f0f, but still pretty bright)
		};
		
		const randVal = () => { //random value between 300 and 510
		...
		rescale = (x) => { //now, rescale the random numbers so that our output color has the luminosity we want
			return x*luminosity/thisColor.reduce((a,b) => {return a+b;} ) //sum red, green, and blue to get the total luminosity
		};
		thisColor = fixDifference(thisColor.map((a) => {return rescale(a);})); //fix the hue so that our color is recognizable
    ...
    for (i = loadedColors.length; i < number; i++) { //Start with our predefined colors or 0, and generate the correct number of colors.
        loadedColors.push(color().map((value) => { //for each new color
