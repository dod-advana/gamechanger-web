...
describe('SearchUtility', function () {
	...
	describe('#createCacheKeyFromOptions()', () => {
		const utility = new SearchUtility(constructorOptionsMock);
	...
	describe('#getESSuggesterQuery()', function () {
		...
		it('should create the ElasticSearch suggester query from searchText', () => {
			...
			let target = new SearchUtility(constructorOptionsMock);
	...
	describe('#getESSuggesterPresearchQuery()', function () {
		...
		it('should create the ElasticSearch suggester query from searchText', () => {
			...
			const expected = [{index: 'gamechanger'}, {_source: ['filename'], query: {wildcard: {filename: {boost: 1, rewrite: 'constant_score', value: '*MISSION*'}}}, size: 2}, {index: 'gamechanger'}, {_source: ['title'], query: {wildcard: {title: {boost: 1, rewrite: 'constant_score', value: '*MISSION*'}}}, size: 2}, {index: undefined}, {aggs: {search_query: {aggs: {user: {terms: {field: 'user_id', size: 3}}}, terms: {field: 'search_query', min_doc_count: 5}}}, query: {prefix: {search_query: {value: 'mission'}}}, size: 1}, {index: undefined}, {_source: {includes: ['name', 'aliases']}, query: {prefix: {name: {value: 'mission'}}}, size: 1}];
			assert.deepStrictEqual(actual, expected);
		});
	...
	describe('#getElasticsearchQuery', () => {
		it('should return an es query given parsedQuery and defaults', () => {
			...
			const expected = {_source: {includes: ['pagerank_r', 'kw_doc_score_r', 'orgs_rs', 'topics_rs']}, aggregations: {doc_org_aggs: {terms: {field: 'display_org_s', size: 10000}}, doc_type_aggs: {terms: {field: 'display_doc_type_s', size: 10000}}}, from: 0, query: {bool: {must: [{nested: {inner_hits: {_source: false, from: 0, highlight: {fields: {'paragraphs.filename.search': {number_of_fragments: 0}, 'paragraphs.par_raw_text_t': {fragment_size: 180, number_of_fragments: 1}}, fragmenter: 'span'}, size: 5, stored_fields: ['paragraphs.page_num_i', 'paragraphs.filename', 'paragraphs.par_raw_text_t']}, path: 'paragraphs', query: {bool: {should: [{wildcard: {'paragraphs.filename.search': {boost: 15, value: 'artificial intelligence*'}}}, {query_string: {default_field: 'paragraphs.par_raw_text_t', default_operator: 'and', fuzziness: 'AUTO', fuzzy_max_expansions: 100, query: 'artificial intelligence', }}]}}}}, {query_string: {query: 'is_revoked_b:false'}}, {query_string: {query: 'display_org_s:(undefined) AND display_doc_type_s:(undefined)'}}], should: [{multi_match: {fields: ['keyw_5^2', 'id^2', 'summary_30', 'paragraphs.par_raw_text_t'], operator: 'or', query: 'artificial intelligence'}}, {rank_feature: {boost: 0.5, field: 'pagerank_r'}}, {rank_feature: {boost: 0.1, field: 'kw_doc_score_r'}}]}}, size: 20, stored_fields: ['filename', 'title', 'page_count', 'doc_type', 'doc_num', 'ref_list', 'id', 'summary_30', 'keyw_5', 'p_text', 'type', 'p_page', 'display_title_s', 'display_org_s', 'display_doc_type_s', 'is_revoked_b', 'access_timestamp_dt', 'publication_date_dt', 'crawler_used_s'], track_total_hits: true};
			assert.deepStrictEqual(actual, expected);
		});
	});
...
;
	...
	describe('#getElasticsearchQueryUsingDocId', () => {
		it('should return an es query given docIds', () => {
			const opts = {
				...constructorOptionsMock,
				constants: {env: { GAME_CHANGER_OPTS: {allow_daterange: false}}}
			};
			...
			const parsedQuery = 'artificial intelligence';
			const docIds = ['Test', 'Test2'];
			let target = new SearchUtility(opts);

			const actual = target.getElasticsearchQueryUsingDocId({
				docIds,
				searchText: 'test',
				searchTerms: 'test',
				parsedQuery
			});
			const expected = {_source: {includes: ['pagerank_r', 'kw_doc_score_r', 'pagerank', 'topics_rs']}, stored_fields: ['filename', 'title', 'page_count', 'doc_type', 'doc_num', 'ref_list', 'id', 'summary_30', 'keyw_5', 'type', 'pagerank_r', 'display_title_s', 'display_org_s', 'display_doc_type_s'], track_total_hits: true, size: 100, query: {bool: {must: {terms: {id: ['Test', 'Test2']}}, should: {nested: {path: 'paragraphs', inner_hits: {_source: false, stored_fields: ['paragraphs.page_num_i', 'paragraphs.filename', 'paragraphs.par_raw_text_t'], from: 0, size: 5, highlight: {fields: {'paragraphs.filename.search': {number_of_fragments: 0}, 'paragraphs.par_raw_text_t': {fragment_size: 180, number_of_fragments: 1}}, fragmenter: 'span'}}, query: {bool: {should: [{wildcard: {'paragraphs.filename.search': {value: 'artificial intelligence*', boost: 15}}}, {query_string: {query: 'artificial intelligence', default_field: 'paragraphs.par_raw_text_t', default_operator: 'and', fuzzy_max_expansions: 100, fuzziness: 'AUTO'}}]}}}}}}};
			assert.deepStrictEqual(actual, expected);
		});
	});
