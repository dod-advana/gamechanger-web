...
describe('feedbackController', function () {
	...
	describe('#requestDocIngest', () => {
		const opts = {
			...constructorOptionsMock,
			dataApi: {}
		};
		it('should post a request for a document to be ingested', async (done) => {
			try {
				const doc_ingest_request = {
					findOrCreate() {
						return Promise.resolve([{doc: 'test'}, true]);
					}
				};
				...
				const target = new FeedbackController(newOpts);
				
				await target.requestDocIngest(req, res);
