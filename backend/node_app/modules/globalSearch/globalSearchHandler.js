...
const { QLIK_URL, QLIK_WS_URL, CA, KEY, CERT, AD_DOMAIN, QLIK_SYS_ACCOUNT } = constants.QLIK_OPTS;
...
class GlobalSearchHandler extends SearchHandler {
	...
	async searchHelper(req, userId) {
		...
		try {
			...
			// Applications
			if (getApplications) {
				searchResults.applications = await getApplicationResults(searchText, offset, limit, userId);
			}

			// Dashboards
			if (getDashboards) {
				searchResults.dashboards = await getDashboardResults(searchText, offset, limit, userId);
			}

			// Data Sources
			if (getDataSources) {
				searchResults.dataSources = await getDataCatalogResults(searchText, offset, limit, 'dataSources', userId);
			}

			// Databases
			if (getDataSources) {
				searchResults.databases = await getDataCatalogResults(searchText, offset, limit, 'databases', userId);
			}
			...
			try {
				...
				LOGGER.error(e.message, 'BG3V9N8', userId);
			}
			...
			const { message } = err;
			LOGGER.error(message, 'VU2YLNH', userId);
			historyRec.endTime = new Date().toISOString();
...
const getApplicationResults = async (searchText, offset, limit, userId) => {
	try {
		...
		const [apps, appResults] = performApplicationSearch(results, lunrSearchUtils.parse(searchText));
		return generateRespData(apps, appResults, offset, limit);
	} catch (err) {
		LOGGER.error(err, 'MJ2D6VG', userId);
		return { hits: [], totalCount: 0, count: 0 };
	}
};

const getDashboardResults = async (searchText, offset, limit, userId) => {
	try {
		let results = await Promise.all([getQlikApps(), getQlikApps({}, userId.substring(0, userId.length - 4))]);

		const [apps, searchResults] = performSearch(mergeUserApps((results[0].data || []), (results[1].data || [])), lunrSearchUtils.parse(searchText));
		return generateRespData(apps, searchResults, offset, limit);
	} catch (err) {
		LOGGER.error(err, 'WS18EKR', userId);
		return { hits: [], totalCount: 0, count: 0 };
	}
};

const getDataCatalogResults = async (searchText, offset, limit, searchType = 'all', userId) => {
	...
	try {
		...
		LOGGER.error(err, 'FE656U9', UserId);
		return [];
	}
};

const performApplicationSearch = (allApps, searchText) => {
...
const generateRespData = (apps, searchResults, offset, limit) => {
	try {
		let ret = [];
		...
		LOGGER.error(e, 'QW8UGJM');
		return { hits: [], totalCount: 0, count: 0 };
	}
};

const getQlikApps = async (params = {}, userId) => {
	try {
		let url = `${QLIK_URL}/qrs/app/full`;
		let result = await axios.get(url, getRequestConfigs({ filter: APP_PROD_FILTER, ...params }, userId));
		return result;
	} catch (err) {
...
const getRequestConfigs = (params = {}, userid = QLIK_SYS_ACCOUNT) => {
	return {
		...
		headers: { 'content-type': 'application/json', 'X-Qlik-xrfkey': '1234567890123456', 'X-Qlik-user': getUserHeader(userid) },
...
const performSearch = (allApps, searchText) => {
	// create a map of apps keyed by id field so that we can reference them from the search results
...
const mergeUserApps = (apps, userApps) => {
	for (let app of apps) {
...
// const globalSearchHandler = new GlobalSearchHandler();
