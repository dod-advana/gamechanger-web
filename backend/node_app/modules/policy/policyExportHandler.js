...
class PolicyExportHandler extends ExportHandler {
	constructor(opts = {}) {
		const { constants = constantsFile, app_settings = APP_SETTINGS, mlApi = new MLApiClient(opts) } = opts;
		super();
		...
		try {
			...
			try {
				...
				if (intelligentSearchOn) {
					...
					const intelligentSearchFound = searchResults.docs.findIndex(
						(item) => item.id === intelligentSearchResult.id
					);
					if (intelligentSearchFound !== -1) {
						// if found, remove that entry from list
			...
			try {
				...
				docs.forEach((d) => {
					if (
						...
						selectedDocuments.indexOf(d.filename) !== -1
					) {
						cleanedDocs.push(d);
					}
				});
