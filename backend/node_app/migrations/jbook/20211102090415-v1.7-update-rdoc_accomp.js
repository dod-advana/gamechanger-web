	...
	up: (queryInterface, Sequelize) => {
		return queryInterface.describeTable(tablename).then((tableDefinition) => {
			...
			if (tableDefinition['project_number'])
				queries.push(queryInterface.renameColumn(tablename, 'project_number', 'Project_Number'));
			if (tableDefinition['search'])
				queries.push(queryInterface.renameColumn(tablename, 'search', 'Accomp_Fund_PY_Text_Search'));
