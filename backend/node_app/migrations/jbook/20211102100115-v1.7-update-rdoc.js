...
module.exports = {
	up: (queryInterface, Sequelize) => {
		return queryInterface.describeTable(tablename).then(tableDefinition => {
			...
			if (!tableDefinition['Accomp_Fund_PY_Text']) queries.push(queryInterface.addColumn(tablename, 'Accomp_Fund_PY_Text', Sequelize.TEXT));
			if (!tableDefinition['PlanPrgrm_Fund_BY1_Text']) queries.push(queryInterface.addColumn(tablename, 'PlanPrgrm_Fund_BY1_Text', Sequelize.TEXT));
			if (!tableDefinition['PlanPrgrm_Fund_BY1Base_Text']) queries.push(queryInterface.addColumn(tablename, 'PlanPrgrm_Fund_BY1Base_Text', Sequelize.TEXT));
			if (!tableDefinition['PlanPrgrm_Fund_BY1OCO_Text']) queries.push(queryInterface.addColumn(tablename, 'PlanPrgrm_Fund_BY1OCO_Text', Sequelize.TEXT));
			if (!tableDefinition['PlanPrgrm_Fund_CY_Text']) queries.push(queryInterface.addColumn(tablename, 'PlanPrgrm_Fund_CY_Text', Sequelize.TEXT));
			if (!tableDefinition['Accomp_Fund_PY_Text_Search']) queries.push(queryInterface.addColumn(tablename, 'Accomp_Fund_PY_Text_Search', Sequelize.TSVECTOR));
			if (!tableDefinition['Adj_OtherAdj_Title_Search']) queries.push(queryInterface.addColumn(tablename, 'Adj_OtherAdj_Title_Search', Sequelize.TSVECTOR));
			if (!tableDefinition['Appn_Title_Search']) queries.push(queryInterface.addColumn(tablename, 'Appn_Title_Search', Sequelize.TSVECTOR));
			if (!tableDefinition['BA_Title_Search']) queries.push(queryInterface.addColumn(tablename, 'BA_Title_Search', Sequelize.TSVECTOR));
			if (!tableDefinition['CongAdds_Title_Search']) queries.push(queryInterface.addColumn(tablename, 'CongAdds_Title_Search', Sequelize.TSVECTOR));
			if (!tableDefinition['Event_Title_Search']) queries.push(queryInterface.addColumn(tablename, 'Event_Title_Search', Sequelize.TSVECTOR));
			if (!tableDefinition['JointFund_Title_Search']) queries.push(queryInterface.addColumn(tablename, 'JointFund_Title_Search', Sequelize.TSVECTOR));
			if (!tableDefinition['OthProgFund_Title_Search']) queries.push(queryInterface.addColumn(tablename, 'OthProgFund_Title_Search', Sequelize.TSVECTOR));
			if (!tableDefinition['PE_MSN_Dsec_Search']) queries.push(queryInterface.addColumn(tablename, 'PE_MSN_Dsec_Search', Sequelize.TSVECTOR));
			if (!tableDefinition['PE_Title_Search']) queries.push(queryInterface.addColumn(tablename, 'PE_Title_Search', Sequelize.TSVECTOR));
			...
			if (!tableDefinition['Proj_Title_Search']) queries.push(queryInterface.addColumn(tablename, 'Proj_Title_Search', Sequelize.TSVECTOR));
			if (!tableDefinition['SubProj_Title_Search']) queries.push(queryInterface.addColumn(tablename, 'SubProj_Title_Search', Sequelize.TSVECTOR));
