  ...
  up: (queryInterface, Sequelize) => {
    return queryInterface.describeTable(tablename).then(tableDefinition => {
        const queries = [];
         ...
         if (!tableDefinition['preferred_name']) queries.push(queryInterface.addColumn(tablename, 'preferred_name', Sequelize.TEXT));
         if (!tableDefinition['sub_office']) queries.push(queryInterface.addColumn(tablename, 'sub_office', Sequelize.TEXT));
         if (!tableDefinition['country']) queries.push(queryInterface.addColumn(tablename, 'country', Sequelize.TEXT));
         ...
         if (!tableDefinition['job_title']) queries.push(queryInterface.addColumn(tablename, 'job_title', Sequelize.TEXT));
