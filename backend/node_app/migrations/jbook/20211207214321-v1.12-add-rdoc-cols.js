...
module.exports = {
	up: (queryInterface, Sequelize) => {
		return queryInterface.describeTable(tablename).then((tableDefinition) => {
			...
			if (!tableDefinition['Proj_MSN_Desc'])
				queries.push(queryInterface.addColumn(tablename, 'Proj_MSN_Desc', Sequelize.TEXT));
			if (!tableDefinition['Proj_Notes'])
				queries.push(queryInterface.addColumn(tablename, 'Proj_Notes', Sequelize.TEXT));
			if (!tableDefinition['Proj_Aquisition_Startegy'])
				queries.push(queryInterface.addColumn(tablename, 'Proj_Aquisition_Startegy', Sequelize.TEXT));
			if (!tableDefinition['Proj_Performance_Metrics'])
				queries.push(queryInterface.addColumn(tablename, 'Proj_Performance_Metrics', Sequelize.TEXT));
			if (!tableDefinition['OthProgFund_Summary_Remarks'])
				queries.push(queryInterface.addColumn(tablename, 'OthProgFund_Summary_Remarks', Sequelize.TEXT));
