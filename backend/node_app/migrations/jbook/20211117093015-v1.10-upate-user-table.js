  ...
  up: (queryInterface, Sequelize) => {
    return queryInterface.describeTable(tablename).then(tableDefinition => {
        const queries = [];
         ...
         if (tableDefinition['middle_initial']) queries.push(queryInterface.removeColumn(tablename, 'middle_initial', Sequelize.TEXT));
         if (!tableDefinition['user_id']) queries.push(queryInterface.addColumn(tablename, 'user_id', {type: Sequelize.TEXT, unique: true}));
