...
class UserController {
	...
	async sendClassificationAlert(req, res) {
		...
		try {
			userId = req.get('SSL_CLIENT_S_DN_CN');
			const { classificationMarking } = req.body.alertData;
			const emailBody = `<p>A user with ID ${userId} has exported their search results with a non-standard classification marking.</p><p>The marking they used is: ${classificationMarking}</p>`;
